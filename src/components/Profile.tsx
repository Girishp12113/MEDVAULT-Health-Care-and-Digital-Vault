    ...
    const loadProfile = async () => {
      try {
        ...
        if (metadata.profileData) {
          console.log('Found profile data in metadata');
        ...
        if (savedProfile) {
          try {
            ...
            try {
              ...
              console.log('Profile saved to metadata');
            } catch (metaError) {
              console.log('Could not save to metadata:', metaError);
            }
            setIsLoading(false);
          } catch (parseError) {
            console.error('Error parsing localStorage data:', parseError);
            setComponentError('Error loading saved profile data');
            setIsLoading(false);
          }
        } else {
          // If no data found anywhere, create basic profile with email
          console.log('No profile data found, creating basic profile with email');
          setProfile(prev => ({
            ...prev,
            email: user.email || ''
          }));
        ...
        setComponentError(`Error loading profile: ${error instanceof Error ? error.message : 'Unknown error'}`);
