...
import { FileText, Calendar, Pill as Pills, Activity, User, LogOut, Brain, Users, ClipboardList, Heart, Shield } from 'lucide-react';
...
const Navbar = ({ userRole, onLogout }: NavbarProps) => {
  ...
  const patientNavItems = [
    ...
    },
    {
      ...
      description: 'Manage doctor access requests with OTP verification'
    },
    {
      path: '/profile',
      icon: User,
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`inline-flex items-center px-2 pt-1 border-b-2 text-sm font-medium min-w-0 ${
                      isActive
