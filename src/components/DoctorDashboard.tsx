...
import { Bell, User, Users, Activity, Calendar } from 'lucide-react';
    ...
    const fetchPatients = async () => {
      try {
        ...
        const { data: reportsData, error: reportsError } = await supabase
          .from('reports')
          .select('id, patient_id, report_name, uploaded_at, report_type');
  ...
  return (
    <div className="relative min-h-screen bg-gray-100">
      ...
      <div className="container mx-auto px-4 py-8">
                            ...
                            <ul className="list-disc list-inside">
                              {patient.reports.slice(0, 2).map(report => (
                                <li key={report.id} className="truncate">
                                  {report.report_name} ({new Date(report.uploaded_at).toLocaleDateString()})
                                </li>
                              ))}
                            </ul>
                          </div>
        ...
        {patients.length === 0 && !error && activeTab === 'overview' && (
